    ...
    <div class="dropdown-box">
        @if(isset($cart) && count($cart) > 0)
                    ...
                    @if ($product != null)
                        <div class="product product-cart">
                            <div class="product-detail">
                                <a href="{{ route('product', $product->slug) }}" class="product-name">{{  $product->getTranslation('name')  }}</a>
                                <div class="price-box">
                                    <span class="product-quantity">{{ $cartItem['quantity'] }}</span>
                                    <span class="product-price">${{ single_price($cartItem['price']) }}</span>
                                </div>
                            </div>
                            <figure class="product-media">
                                <a href="{{ route('product', $product->slug) }}">
                                    <img src="{{ uploaded_asset($product->thumbnail_img) }}"  alt="{{  $product->getTranslation('name')  }}" height="84"
                                        width="94" />
                                </a>
                            </figure>
                            <button class="btn btn-link btn-close" onclick="removeFromCart({{ $cartItem['id'] }})" aria-label="button">
            ...
            <div class="cart-action">
                ...
                @if (Auth::check())
                    <a href="{{ route('checkout.shipping_info') }}" class="btn btn-primary  btn-rounded">Checkout</a>
                @endif
            </div>
        @else
